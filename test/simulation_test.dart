...
void main() {
  ...
  test(
    'quest log notes round-trip and append without mutating the default',
    () {
  ...
  test(
    'a keep-until-done to-do lingers across days, then clears once done',
    () {
      today = DateTime(2026, 6, 15);
      final s = GameState();
  ...
  test('domain notes and goal journals round-trip through the save', () {
    ...
    s.setDomainNotes(
      ...
      s
          .notesFor(Stat.dis)
          .withNote('declutter the desk', today, context: 'Cluttered'),
    );
    s.addGoal(Goal(title: 'Keep the home', stat: Stat.dis, target: 25));
    s.goals.first.notes = s.goals.first.notes.withNote(
      'week one, kitchen done',
      today,
