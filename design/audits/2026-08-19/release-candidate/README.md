...
### Task 3 evidence repair: narrow About and Room Guide

| Command | Exit code | Definitive footer | Output |
| --- | --- | --- | --- |
| `flutter test --update-goldens --dart-define=CAPTURE_GOLDENS=true test/screenshots_test.dart --plain-name "about screen narrow large text"` | 0 | `00:00 +1: All tests passed!`; `TASK3_FIX_ABOUT_EXIT=0` | `test/goldens/about_screen_narrow_320x568_2x.png`; `test/goldens/about_screen_narrow_scrolled_320x568_2x.png` |
| `flutter test --update-goldens --dart-define=CAPTURE_GOLDENS=true test/room_guide_test.dart --plain-name "guide remains usable on a narrow large-text phone"` | 0 | `00:00 +1: All tests passed!`; `TASK3_FIX_GUIDE_EXIT=0` | `test/goldens/room_guide_narrow_320x568_text_1_3x.png`; `test/goldens/room_guide_narrow_scrolled_320x568_text_1_3x.png` |
| Four-file `System.Drawing.Image.FromFile` decode/dimension check | 0 | `TASK3_FIX_DECODE_EXIT=0` | All four decode at exactly 320 x 568. |

All four files above were opened individually. At 200-percent text, About's title wraps intentionally, the top card preserves reading order, and the scrolled state exposes both lower support actions plus the free-use promise without horizontal clipping or a lost target. At 130-percent text, Room Guide keeps its intro and first door readable at the top, while the scrolled capture exposes the complete `Me` door and closing guidance without clipping or inset collision. Fixed headers remain visually distinct from the scrolling content in both states. No additional BLOCKER, HIGH, or FINISH finding is warranted. The masked `large_text_*` frames were used only to judge geometry and reflow, never to infer copy or type defects.
