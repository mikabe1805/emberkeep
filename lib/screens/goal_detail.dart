...
class GoalDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final related =
        quests.where((q) => q.goalTitle == goal.title).toList(growable: false);
    final now = Clock.now();
    ...
    final activeToday =
        related.where((q) => q.snoozedDay != todayKey).toList(growable: false);
              ...
              Expanded(
                child: ListView(
                  ...
                  children: [
                    ...
                    _statTiles(context, related.length, doneToday,
                        activeToday.length, days),
                    const SizedBox(height: 16),
                    _relatedPanel(
                        context, related, now, doneToday, activeToday.length),
  ...
  // ── header: back · title · kind pill ──────────────────────────────
  Widget _header(BuildContext context) {
    final kindLabel = goal.complete
        ? 'ACHIEVED'
        ...
        children: [
          ...
          Expanded(
            child: Text(goal.title,
                ...
                style: Type.display.copyWith(fontSize: 24, color: _accent)),
          ),
            ...
            builder: (_, t, _) => SizedBox(
              ...
              child: CustomPaint(
                painter: _RingPainter(
                    value: _ringValue * t, color: _accent, complete: goal.complete),
                child: Center(
                  child: goal.complete
  ...
  // ── the cool stats ────────────────────────────────────────────────
  Widget _statTiles(BuildContext context, int active, int doneToday,
      int doneDenom, int? days) {
    final tiles = <Widget>[
      _tile(context, 'QUESTS COMPLETED', '${goal.progress}',
          Icons.check_circle_outline),
      _tile(context, 'DONE TODAY', '$doneToday / $doneDenom',
          Icons.local_fire_department),
      _tile(context, 'ACTIVE QUESTS', '$active', Icons.bolt),
      if (goal.kind == GoalKind.become)
        _tile(context, 'MILESTONES REACHED', '${goal.milestones}',
            Icons.all_inclusive)
      else
        _tile(
            context,
            ...
            Icons.flag),
      _tile(context, 'DAYS ON THE JOURNEY', days == null ? '—' : '$days',
          Icons.wb_twilight),
    ];
  ...
  Widget _relatedPanel(BuildContext context, List<Quest> related, DateTime now,
      int doneToday, int denom) {
        ...
        child: Row(
          children: [
            Container(
              ...
              height: 8,
              decoration:
                  BoxDecoration(shape: BoxShape.circle, color: q.stat.color),
            ),
            ...
            Icon(
              ...
              size: 18,
              color: done ? Palette.success : q.stat.color.withValues(alpha: 0.6),
            ),
          ],
        ),
      ),
...
class _RingPainter extends CustomPainter {
  _RingPainter({required this.value, required this.color, required this.complete});
  final double value;
  final Color color;
  final bool complete;
